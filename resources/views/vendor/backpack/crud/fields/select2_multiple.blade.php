    ...
    <select
        ...
        @if (isset($field['model']))
            ...
            if (isset($field['filter']) && is_array($field['filter'])) {
                ...
                $fieldss = $field['model']::where($key, $op, $val)->get();
            } else {
                $fieldss = $field['model']::all();
            }
            ?>

            @foreach ($fieldss as $connected_entity_entry)

                <?php
                $attrs = explode("|", $field['attribute']);
                $title = '';
                foreach ($attrs as $attr){
                    $title = $title . ' ' .  $connected_entity_entry->$attr;
                }
                ?>
                ...
                @else
                    <option value="{{ $connected_entity_entry->getKey() }}">{{ $title }}</option>
                @endif
            @endforeach
        @endif
    </select>
