...
@section('header')
    <section dir="rtl" class="content-header" style="padding-top: 15px">
        <h1 style="text-align: right; font-size: 22px;">
            <span class="text-capitalize">دانش آموزان کلاس <strong>{{$course->title}}</strong></span>
        </h1>
    </section>
@endsection
...
@section('after_scripts')
    <script>
        ...
        function request_add_students_to_course() {
            $.ajax({
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type: "POST",
                url: '{{ url("/add_students_to_course") }}',
                data: {
                    course_id: '{{$course->id}}',
                    id_list: course_students,
                },
        ...
        function request_remove_students_from_course() {
            $.ajax({
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type: "POST",
                url: '{{ url("/remove_students_from_course") }}',
                data: {
                    course_id: '{{$course->id}}',
                    id_list: selected_course_students,
                },
                success: function(data)
                {
                    console.log(data);
        ...
        $('#btn_add').click(function () {
            ...
            request_add_students_to_course();
        });
        ...
        $('#btn_remove').click(function () {
            ...
            request_remove_students_from_course();
        });
